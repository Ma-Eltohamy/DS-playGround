...
public:
  ...
  void sort() { // Then Time O(n logn)
    const int SIZE = size;

    while (size--) // O(n Touches every element in the arr)
      std::swap(arr[0], arr[size]), heapifyDown(0); // O(log n heapifyDown)
...
void heap_sort() {
  ...
  std::vector<int> v{2, 17, 22, 10, 8, 37, 14, 19, 7, 6, 5, 12, 25, 30, 9};
  MinHeap heap(v);
