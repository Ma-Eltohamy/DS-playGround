...
class Solution {
public:
  void _parenthazingLeftTree(std::string &res, TreeNode *root) {
    if (!root) {
      res += "()";
      return;
    }
    res += "(" + std::to_string(root->val);
    if (root->left)
      _parenthazingLeftTree(res, root->left);
    else
      res += "()";
    if (root->right)
      _parenthazingLeftTree(res, root->right);
    else
      res += "()";
    res += ')';
  }
  void _parenthazingRightTree(std::string &res, TreeNode *root) {
    if (!root) {
      res += "()";
      return;
    }
    res += "(" + std::to_string(root->val);
    if (root->right)
      _parenthazingRightTree(res, root->right);
    else
      res += "()";
    if (root->left)
      _parenthazingRightTree(res, root->left);
    else
      res += "()";
    res += ')';
  }
  ...
  bool isSymmetric(TreeNode *root) {
    ...
    _parenthazingLeftTree(res1, root->left);
    _parenthazingRightTree(res2, root->right);
    if (res1 == res2)
      return true;
    return false;
  }
