...
class Solution {
public:
  TreeNode *GenerateFullBT(std::queue<std::pair<int, bool>> &preOrderQueue) {

    std::pair<int, int> p = preOrderQueue.front();
    preOrderQueue.pop();

    TreeNode *root{new TreeNode(p.first)};
    if (!p.second) {
      root->left = GenerateFullBT(preOrderQueue);
      root->right = GenerateFullBT(preOrderQueue);
    }
    return root;
  }
};
