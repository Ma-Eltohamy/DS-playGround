...
public:
  ...
  int max(Node *cur) {}
  // int max(Node *cur = nullptr, bool firstCall = true) { // O(n) time O(n)
  // memory
  //   if (firstCall)
  ...
  ///////////////////////////// new function /////////////////////////////
  void removeAllRepeatedFromSorted() {
    if (!head || head == tail)
      return;

    insertFront(-9784719); // dummy data
    bool deletePrev{false};
    Node *beforePrev{head};
    for (Node *prev{head->next}, *cur{prev->next}; cur; cur = prev->next) {
      ...
      if (deletePrev) {
        deleteNextNode(beforePrev);
        deletePrev = false, prev = cur;
      } else {
        beforePrev = prev, prev = cur;
      }
    }
    if (deletePrev)
      deleteNextNode(beforePrev), deletePrev = false;
    deleteFront();
    debugIntegrityCheck();
  }
  ///////////////////////////// new function /////////////////////////////
  ...
  bool ordering(Node *node) {
    return !node || !node->next ||
           node->data > node->next->data && ordering(node->next);
  }
...
void test1() {
  ...
  list.removeAllRepeatedFromSorted();
  list.print();
