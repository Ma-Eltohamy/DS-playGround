...
public:
  ...
  ///////////////////////////// new function /////////////////////////////
  // int max(Node *cur) {}
  // int max(Node *head = nullptr, bool isFirst = true) { // O(n) time O(n)
  // memory
  //   // we could use the head ptr to know if it is the first time to
  //   // call but sometimes it coluld be nullptr also so it's not uniqe
  //   if (isFirst) {
  //     isFirst = false;
  //     head = this->head;
  //   }
  ...
  //   if (!head)
  //     return -1;
  //   int prevMax{max(head->next, isFirst)};
  //   if (head->data > prevMax)
  //     return head->data;
  //   return prevMax;
  //   // return max(head->next, isFirst);
  //   // return std::max(head->data, max(head->next, isFirst));
  // }
  ///////////////////////////// new function /////////////////////////////
  void oddPosEvenPos() {
  ...
  Node *partition(Node *head, int x) {
    ...
    Node *lowerHead{}, *lower{}, *upperHead{}, *upper{};

    for (Node *cur{head}; cur; cur = cur->next) {
      if (cur->data < x)
        if (!lowerHead)
          lowerHead = cur, lower = lowerHead;
        else
          lower->next = cur, lower = lower->next;
      else {
        if (!upperHead)
          upperHead = cur, upper = upperHead;
        else
          upper->next = cur, upper = upper->next;
      }
    }
