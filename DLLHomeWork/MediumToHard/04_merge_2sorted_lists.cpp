...
public:
  ...
  void reverse() { // O(n) time O(1) memory
    ...
    for (Node *cur{head}, *next{head->next}; cur;) {
      link(cur, cur->prev);
      cur = next;
      if (!next)
        continue;
  ...
  ///////////////////////////// new function /////////////////////////////
  void mergeLists(LinkedList &other) { // O(n + m) time memory O(1)
    if (!other.head)
      return;
    ...
    if (!head)
      ...
      while (cur && otherCur) {
        if (cur->data >= otherCur->data) { // insertion will happen
          other.head = other.head->next;
          link(cur->prev, otherCur), link(otherCur, cur);
          otherCur = other.head;
          if (cur == head)
            head = head->prev;
        } else
      ...
      if (other.head)
        link(tail, other.head), tail = other.tail;
    }
...
void test2() {
  ...
  std::string expected = "5 10 17 20 22 24 30 40 50 60";
  std::string result = list1.debugToStr();
  if (expected != result) {
    std::cout << "no match:\nExpected: " << expected << "\nResult  : " << result
              << "\n";
...
void test3() {
  ...
  std::string expected = "10 20 30 40 50 60";
  std::string result = list1.debugToStr();
  if (expected != result) {
    std::cout << "no match:\nExpected: " << expected << "\nResult  : " << result
              << "\n";
